...
LOCATION_INFO = {
    ...
    "jeddah" : ["21.5292", "39.1611", "id=14&key=d486da6d64f8571a964d96365fe395cd"],
    "dhahran" : ["26.2361", "50.0393", "id=15&key=7b1a65c9cb29b9d1eabbbb1d5e4e60fb"]
}
...
class App:
    ...
    def save_prayer_times(location: str) -> None:
        timings = App.get_prayer_times(location)
    ...
    def check(server_ip: str, location: str):
        ...
        for timing in timings:
            ...
            # Activate the trigger 1 minute before the prayer time
            trigger_dt = timing - datetime.timedelta(minutes=1)

            if trigger_dt == current_time:
                App.activate(server_ip, location)
...
def main():
    ...
    if not event_debug:
        ...
        if operation == "save":
            App.save_prayer_times(location)
        
        if operation == "get":
            print(App.get_prayer_times(location))

        if operation == "check":
            App.check(server_ip, location)
